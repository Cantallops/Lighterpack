...
final class LibraryStore: ObservableObject {
    ...
    @Published private(set) var lists: [Entities.List] = []
    @Published private(set) var categories: [Entities.Category] = []
}

extension LibraryStore {
    func categories(ofList list: Entities.List) -> [Entities.Category] {
        categories(ofList: list, using: categories)
    }

    func categories(ofList list: Entities.List, using categories: [Entities.Category]) -> [Entities.Category] {
        categories.filter {
            list.categoryIds.contains($0.id)
        }
    }

    func category(withId id: Int) -> Entities.Category? {
        categories.first { $0.id == id }
    }

    func remove(category: Entities.Category) {
        categories.removeAll(where: { $0.id == category.id })
        recompute()
    }

    func remove(categoryItems: [CategoryItem], in category: Entities.Category) {
    ...
    func move(categoryItem: CategoryItem, from source: Entities.Category, to destiantion: Entities.Category) {
...
extension LibraryStore {
    func update(with library: Library) {
        ...
        self.status = .loaded(.init())
    }

    private func recompute(
        items: [Item]? = nil,
        categories: [Entities.Category]? = nil,
        lists: [Entities.List]? = nil
    ) {
...
extension LibraryStore {
    ...
    func replace(list: Entities.List) {
        defer {
            self.status = .unsync
        }
    ...
    func replace(categoryItem: CategoryItem, in category: Entities.Category) {
        defer {
            self.status = .unsync
        }
        var modCategory = category
    ...
    func replace(category: Entities.Category) {
        defer {
            self.status = .unsync
        }
